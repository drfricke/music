...
def FindaverageNote(aveImu, kernel):
    aveImuX, aveImuY, aveImuZ, aveImuGX, aveImuGY, aveImuGZ = 0, 0, 0, 0, 0, 0
    for i in range(0, kernel):
        aveImuX += float(aveImu[i][0])
        aveImuY += float(aveImu[i][1])
        aveImuZ += float(aveImu[i][2])
        aveImuGX += float(aveImu[i][3])
        aveImuGY += float(aveImu[i][4])
        ...
        if len(data) == 1:
            ...
            if len(imu) == 6:
                if float(imu[2]) >= .2:
                    print('rest position')
                if count == kernel:
                    count = 0 #refresh the count
                    playNote = FindaverageNote(aveImu, kernel) #see NEW function
                    print(playNote)
                    aveImu = zeroListMaker(kernel) #refresh note collection
